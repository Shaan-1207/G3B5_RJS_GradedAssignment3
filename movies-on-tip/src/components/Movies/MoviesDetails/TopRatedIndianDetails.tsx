...
import { getTopRatedIndianById } from "../../../services/getMovies";
...
const TopRatedIndianDetails = () => {
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      try {
        ...
        const data = await getTopRatedIndianById(id);
        setMovie(data);
        setLoading(false);
      } catch (error) {
  ...
  return (
    <>
      ...
      {!loading && !error && movie && (
        <>
          <Link to={`/top-rated-indian`} className="rm-us">
